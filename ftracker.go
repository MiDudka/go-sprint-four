...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	meanSpeedMeterSec := (meanSpeed(action, duration)) * kmhInMsec
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeedMeterSec, 2)/(height/100))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
